...
public class NorSplitter : LogicSplitter
{
    ...
    public int[] GetNotesForBeat(int truthValue)
    {
        if (truthValue == 1)
        {
            // For NOR operation, if the truth value is 1, no notes should be spawned
            return new int[0];
        }
        // If the truth value is 0, we can spawn notes in any non-empty combination of lanes
