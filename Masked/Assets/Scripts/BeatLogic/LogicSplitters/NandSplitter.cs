...
public class NandSplitter : LogicSplitter
{
    ...
    public int[] GetNotesForBeat(int truthValue)
    {
        if (truthValue == 0)
        {
            // For NAND operation, if the truth value is 0, spawn notes in all lanes
        ...
        // If the truth value is 1, we can spawn notes in any combination of lanes except all lanes, so we pick a random non-empty subset
