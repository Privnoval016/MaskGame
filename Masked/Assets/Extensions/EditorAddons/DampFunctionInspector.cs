...
namespace Extensions.EditorAddons
{
    ...
    public class DampFunctionInspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (Event.current.type == EventType.Repaint)
            {
                ...
                EditorGUI.LabelField(
                    new Rect(rect.x + paddingLeft - squareSize,
                        rect.y + rect.height - defaultValueOffset - paddingBottom - squareSize / 2, squareSize,
                        squareSize), "1"); // heigt "1" mark
                EditorGUI.LabelField(
                    new Rect(rect.x + paddingLeft - squareSize,
                        rect.y + rect.height - x_AxisOffset - paddingBottom + (squareSize * 0.2f), squareSize,
                        squareSize), "0"); // height "0" mark
                EditorGUI.LabelField(
                    new Rect(rect.x + rect.width - paddingRight - squareSize,
                        rect.y + rect.height - x_AxisOffset - paddingBottom + (squareSize * 0.2f), squareSize,
        ...
        {
            f = ((SODInfo)target).frequency;
            z = ((SODInfo)target).dampingRatio;
            r = ((SODInfo)target).responseTime;
        }

        private void InitFunction()
        {
            f0 = f = ((SODInfo)target).frequency;
            z0 = z = ((SODInfo)target).dampingRatio;
            r0 = r = ((SODInfo)target).responseTime;
        ...
        private void EvaluateFunction()
        {
            ...
            for (int i = 0; i < evaluationSteps; i++)
            {
                ...
                float x_input = math.remap(0, evaluationSteps - 1, -defaultLength, defaultLength, i);
